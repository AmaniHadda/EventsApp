...
def Affiche(request):
    ...
    return render(request,'Event/Affiche.html',
                  {'ee':evt.order_by("nbe_participant")})    
def Search(request):
    ...
    if request.method == 'POST':
        search_value = request.POST.get('title', '')
        events= Event.objects.filter(title__icontains=search_value)
        return render(request,'Event/Affiche.html',{'ee':events})  
...
def Update(request, title):
    ...
    if request.method == 'GET':
        form = FormEvent(instance=event)
        return render(request, 'Event/Update.html', {'form': form, 'event': event})

    elif request.method == 'POST':
        form = FormEvent(request.POST, request.FILES, instance=event)
        if form.is_valid():
            form.save()
            return redirect('Aff')
        else:
            return render(request, 'Event/Update.html', {'form': form, 'event': event, 'msg_erreur': "Erreur lors de la mise à jour de l'événement"})