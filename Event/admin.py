...
class ParticipantList(admin.SimpleListFilter):
    ...
    def queryset(self, request,queryset):
        if self.value()== '0':
            return queryset.filter(title__exact="amani")
        else:
...
class ParticipationsAdmin(admin.StackedInline): #admin.TabularInline
...
class EventAdmin(admin.ModelAdmin):
    ...
    def evt_participation(self,obj):
        ...
        return count
    ordering=('-title','evt_date') #order
    list_per_page=2 #pagination

    list_filter=('state','category',upcomingEvent,ParticipantList) #n3ayto b des classes fl filter w fl actions nhoto def (fonction)

    
    def set_false(self,request,queryset):
        req=queryset.filter(state=False) #retourne events
        if(req.count()>0):
            messages.error(request,f"{req.count()} events are already marked refused")
        else:
            req=queryset.update(state=False) #retourne nbre events updated
            if req==1:
                message="1 event was "
            else:
                message=f"{req} events were"
